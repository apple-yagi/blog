...
今回、PHP の Unit テストにスナップショットテストを導入しました。スナップショットテストを使用するケースはある程度限られますが、今回のように SQL クエリの出力構造をそのまま検証したいケースや SQL Builder のように差分レビューを重視するテストでは特に効果を発揮します。今後もそうした条件に合致する場面では積極的に活用していきたいと思います。
